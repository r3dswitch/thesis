...
class Vis(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless, 
        ...
        setup_arm_positions(self) # self, init_pos_list, trajectory_list_of_lists
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        object_asset_root = "/home/smondal/Desktop/DexterousHands/affordpose_data/Dataset/Part"
        object_file_name = "mug_8619.urdf"
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.grasp_success[:], self.lift_success[:], self.consecutive_successes[:] = compute_hand_reward(
            ...
            self.max_episode_length, self.object_init_pos, self.object_pos, self.cup_handle_pos, self.wrist_pos, self.arm_hand_ff_pos, self.arm_hand_rf_pos, self.arm_hand_mf_pos, self.arm_hand_th_pos, self.arm_hand_lf_pos,
    ...
    def compute_observations(self):
        ...
        # iprint(self.rigid_body_states.shape)
        self.cup_handle_pos = self.rigid_body_states[:, 32, 0:3] 
        self.cup_handle_rot = self.rigid_body_states[:, 32, 3:7]
        self.cup_handle_pos = self.cup_handle_pos + quat_apply(self.cup_handle_rot[:], to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.09)

        self.cup_base_pos = self.rigid_body_states[:, 30, 0:3]
        self.cup_base_rot = self.rigid_body_states[:, 30, 3:7]
    ...
    def post_physics_step(self):
        ...
        if True:
            ...
            # handle_pos = self.cup_handle_pos + torch.tensor([ -0.04060799,  0.08187634, 0.01939938]) # centroid form centroid_from_obj.py with X and Z interchanged
            for i in range(self.num_envs):
                ...
                # self.add_debug_lines(self.envs[i], self.cup_base_pos[i], self.cup_base_rot[i])
...
def compute_hand_reward(
    ...
    max_episode_length: float, object_init_pos, object_pos, handle_pos, wrist_pos, arm_hand_ff_pos, arm_hand_rf_pos, arm_hand_mf_pos, arm_hand_th_pos, arm_hand_lf_pos,
    actions, max_consecutive_successes: int, av_factor: float, rl_start: int
):
    arm_hand_finger_dist = (torch.norm(handle_pos - arm_hand_ff_pos, p=2, dim=-1) + torch.norm(handle_pos - arm_hand_mf_pos, p=2, dim=-1)
                            + torch.norm(handle_pos - arm_hand_rf_pos, p=2, dim=-1) + torch.norm(handle_pos - arm_hand_th_pos, p=2, dim=-1) + torch.norm(handle_pos - arm_hand_lf_pos, p=2, dim=-1))
