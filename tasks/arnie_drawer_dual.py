...
class ArnieDrawerDual(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        object_asset_file = "mjcf/switch/102872/mobility.urdf"
        ...
        self.right_hand_index = self.gym.get_asset_rigid_body_dict(right_diana_asset)["link_6"]
        self.right_j_eef = self.r_jacobian[:, self.right_hand_index - 1, :, :6]
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.consecutive_successes[:] = compute_hand_reward(
            ...
            self.max_episode_length, self.left_object_pos, self.left_object_rot, self.right_object_pos, self.right_object_pos, self.switch_right_handle_pos, self.switch_left_handle_pos, 
    ...
    def compute_observations(self):
        ...
        self.switch_left_handle_pos = self.rigid_body_states[:, self.num_diana_bodies * 2 + 2, 0:3] # 1: base, 2: link1, 3: link0
        self.switch_left_handle_rot = self.rigid_body_states[:, self.num_diana_bodies * 2 + 2, 3:7]
        ...
        self.switch_right_handle_pos = self.rigid_body_states[:, self.num_diana_bodies * 2 + 5, 0:3] # 4: base, 5: link1, 6: link0
        self.switch_right_handle_rot = self.rigid_body_states[:, self.num_diana_bodies * 2 + 5, 3:7]
    ...
    def pre_physics_step(self, actions):
        ...
        if self.progress_buf[0] < 25: # IK
            ...
            right_u = (right_j_eef_T @ torch.inverse(self.right_j_eef @ right_j_eef_T + lmbda) @ right_dpose).view(self.num_envs, 6)     
                
            left_targets = self.left_diana_dof_pos[:,:6] + left_u.squeeze(-1)
            right_targets = self.right_diana_dof_pos[:,:6] + right_u.squeeze(-1)

            self.cur_targets[:, :6] = tensor_clamp(left_targets, self.diana_dof_lower_limits[:6], self.diana_dof_upper_limits[:6])
            self.cur_targets[:, 27:33] = tensor_clamp(right_targets, self.diana_dof_lower_limits[:6], self.diana_dof_upper_limits[:6])
...
def compute_hand_reward(
    ...
    max_episode_length: float, left_object_pos, left_object_rot, right_object_pos, right_object_rot, switch_right_handle_pos, switch_left_handle_pos,
    ...
    reward = torch.exp(-0.2 * (left_hand_dist_rew + right_hand_dist_rew + up_rew))
    ...
    successes = torch.where(successes == 0, 
                            torch.where(1.4-(switch_right_handle_pos[:, 2] + switch_left_handle_pos[:, 2]) > 0.01, 
